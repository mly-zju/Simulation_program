...
double *csma(int client_num,int load,int side)
{
	int block=side*side;
...
//	double x=client_num/(double)(1000000*5);
//	int tc_num=run_time*x;
	int load_num=load;
	int tc_num=client_num/(5*block);
	//cout<<"tc_num is "<<tc_num<<endl;
	load_num+=tc_num;
	cout<<"load_num is "<<load_num<<endl;
	...
	for(t=0;t<run_time;t++)
	{
		if(t%1000000==0)
		{
			for(i=0;i<client_num;i++)
				...
				{
					dapk[i][j]=0;
					dapk[i][j]=(rand()%10)*100000+(rand()%10)*10000+rand()%10000+t;
					dapk[i][j]=dapk[i][j]/w_slot;
				}
		}
		if(t%w_slot==0)
		{
...
int main()
{
	...
	ofstream outfile;
	outfile.open("data4.txt",ios::app);
	while(load<=300)
	{
		for(i=0;i<3;i++)
			result[i]=0;
