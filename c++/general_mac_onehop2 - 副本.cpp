...
void csma(int client_num, int load)
{
	...
	int load_num=load*client_num*multi;
	int z_load=10*client_num*multi;
	int w_load=load*client_num*multi;
	double z_delay=0;
	double w_delay=0;
	struct packet_map wpk[w_load];
	struct packet_map zpk[z_load];
	ofstream outfile;
	outfile.open("data10.txt",ios::app);
	...
	for(i=0;i<w_load;i++)
	{
		wpk[i].time=(rand()%10)*10000000+(rand()%10)*1000000+(rand()%8)*100000+rand()%100000;
