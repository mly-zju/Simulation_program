...
void csma(int client_num, int load)
{
	...
	int z_load=load*client_num*multi/division;
	int w_load=load*client_num*multi-z_load;
	double z_delay=0;
	double w_delay=0;
	...
	ofstream outfile;
	outfile.open("data11.txt",ios::app);
	...
	for(i=0;i<w_load;i++)
	{
		wpk[i].time=(rand()%10)*10000000+(rand()%10)*1000000+(rand()%8)*100000+rand()%100000+zigbee_time;
	...
	for(i=0;i<z_load;i++)
	{
		zpk[i].time=(rand()%10)*10000000+(rand()%10)*1000000+(rand()%8)*100000+rand()%100000;
		zpk[i].time=zpk[i].time/(division*z_slot);
